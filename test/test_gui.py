...
def models_and_data():
    ...
    pm1 = Parametric1D(sp.exp(tau*x) + alpha, parameters, call_type=pd.Series)
    parameters['tau'] = (0.001, 1.0, 6.0)
    pm2 = Parametric1D(alpha*sp.exp(2j*np.pi*tau*x), parameters, call_type=pd.Series)
...
def test_transforms(models_and_data):
    ...
    ax2 = gui.register_model(pm2, sigma1.index, color='r')
    gui.register_data(sigma2, axis=ax2, color='b')
...
def test_gui_two_model_one_plot(models_and_data):
    ...
    gui.register_data(sigma1, axis=ax, color='b')
    gui.register_data(sigma2, axis=ax, color='b')
